...
def plotObj(obj_list):
    ...
    plt.show()
def outPut(model,iters):
    work=xlsxwriter.Workbook('result_de{}.xlsx'.format(iters))
    ...
    work.close()
def run(filepath,iters,epochs=150, Cr=0.5,F=0.5, popsize=400, v_cap=80, opt_type=1):
    """
    :param filepath: Xlsx file path
    :param epochs: itersations
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--CrossoverRate", help = "set Crossover Rate")
    parser.add_argument("-f", "--ScalingFactor", help = "set Scaling Factor")
    ...
    if args.epochs:
        epochs=args.epochs
        if args.CrossoverRate:
            Cr=args.CrossoverRate
            if args.ScalingFactor:
                F=args.ScalingFactor
                if args.popsize:
                    ...
                    if args.capacity:
                        ...
                        if args.optimize:
                            ...
                            run(file,epochs,Cr,F,popsize,v_cap,opt_type)
    else:
        res=[]
        for i in range(10):
            res.append(run(file,i))
        plotObj(res)
