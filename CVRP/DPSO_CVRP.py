...
def plotObj(obj_list):
    ...
    plt.show()
def outPut(model,iter):
    work=xlsxwriter.Workbook('result_dpso{}.xlsx'.format(iter))
    ...
    work.close()
def run(filepath,epochs,popsize,Vmax,v_cap,opt_type,w,c1,c2,iter):
    """
    ...
    for ep in range(epochs):
        ...
        t=time.time() - start
        print("%s/%s， best obj: %s ,time: %s " % (ep, epochs, model.best_sol.obj,t))
        e_time.append(t)
    print("Average Time Per Epoch: %s" %(sum(e_time)/len(e_time)))
    # plotObj(history_best_obj)
    outPut(model,iter)
    return(history_best_obj[-1])
if __name__=='__main__':
    parser = argparse.ArgumentParser()
    file='D:\daa s4\daa proj\data\CVRP\cvrp.xlsx'
    # parser.add_argument("-e", "--epochs", help = "set Epochs",type=int)
    # parser.add_argument("-vm", "--Vmax", help = "set Max speed")
    # parser.add_argument("-c1", "--param1", help = "set param1")
    # parser.add_argument("-c2", "--param2", help = "set param1")
    # parser.add_argument("-w", "--momenta", help = "set Inertia weight")
    # parser.add_argument("-p", "--popsize", help = "Set Population Size",type=int)
    # parser.add_argument("-v", "--capacity", help = "Set Vehicle capacity",type=int)
    # parser.add_argument("-o", "--optimize", help = "Optimization type:0:Minimize the number of vehicles,1:Minimize travel distance",type=int)
    ...
    #                     v_cap=args.capacity
    #                     if args.optimize:
    #                         opt_type=args.optimize
    #                         if args.momenta:
    #                             w=args.momenta
    #                             run(filepath=file,epochs,popsize,Vmax,v_cap,opt_type,w,c1,c2)
    # else:
    res=[]
    for i in range(10):
        res.append(run(filepath=file,epochs=150,popsize=150,Vmax=2,v_cap=70,opt_type=1,w=0.9,c1=1,c2=5,iter=i))
    plotObj(res)
